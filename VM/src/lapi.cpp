...
lua_State* lua_newthread(lua_State* L, std::string output)
{
    luaC_checkGC(L);
    luaC_checkthreadsleep(L);
    lua_State* L1 = luaE_newthread(L, output);
...
lua_State* lua_newthread(lua_State* L, lua_State* L2)
{
    luaC_checkGC(L);
    luaC_checkthreadsleep(L);
    lua_State* L1 = luaE_newthread(L, L2->output);
    setthvalue(L, L->top, L1);
    api_incr_top(L);
    global_State* g = L->global;
    if (g->cb.userthread)
